...
(function($){
	...
	$.AntriusCtrl = function (options, osdvelt, myanno){
		...
		var handler = options.h ? function(){options.h(options.hparam); myanno.activateSelector();} : function(){myanno.activateSelector();};
...
WebAnnotorious.prototype = {
	...
	get_oneanno: function(as_ratio, aj, baseuri, dimensions, rtype, canvasurl){
		...
		if(!aj.shapes && aj.target){
			...
			if(!frag || (as_ratio && frag.match(/xywh=\d/)) 
				|| (!as_ratio && frag.match(/xywh=percent/) && dim)) frag = this.getfrag(aj, dim, as_ratio);
			text = at.settext(aj);
		}
		...
		var item = {
			"type" : "Annotation",
			"id": aj.id || this.genid(imgurl, frag),
			"bodyValue" : text,
			"target" : {
				"id" : imgurl + "#" + frag,
				"source" : imgurl, 
	...
	pct2ratio: function(frag, dim){
		...
		var res = [];
		frag.forEach(function(f){res.push(Number(f)/100);});
		res[1] /= ar;
		res[3] /= ar;
		return res;
	},
	...
	to_annotorious: function(an, src, dim, imgurl, fraganno){
		...
		var annobj = {
			...
			"id": an.id || this.genid(imgurl, selector),
			"fragid": selector,
